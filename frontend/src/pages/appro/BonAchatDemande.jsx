...
const BonAchatDemande = () => {
  ...
  const canConfirmOrders = isSuperAdmin;
  const isApproAdmin = user?.role === "responsable_appro";
  const canApprove = isSuperAdmin;
  const canCreate = isApproAdmin || isSuperAdmin;
...
const approvedOrdersCount = (purchaseOrders || []).filter(
  (order) => normalizeStatus(order.status) === "approved"
).length;

console.log("📊 DEBUG - Comptes réels:", {
  total: purchaseOrders.length,
  pending: pendingOrdersCount,
  approved: approvedOrdersCount,
  allStatuses: purchaseOrders.map(o => ({ id: o.id, status: o.status, normalized: normalizeStatus(o.status) }))
});
    ...
    try {
      ...
      while (nextUrl) {
        ...
        if (data && data.results) {
          ...
          nextUrl = data.next; // sera null quand on arrive à la fin
          ...
          >
            <Box>
              ...
              <Typography variant="body2" sx={{ color: "#64748b" }}>
                Approuvez ou rejetez les bons d'achat en attente
              </Typography>
            </Box>
              ...
              >
                <Box>
                  <Typography sx={{ color: "#f59e0b", fontWeight: 700 }}>
                    Bons d'achat en attente de confirmation
                  </Typography>
                  <Typography sx={{ color: "#fcd34d", fontSize: "0.9rem" }}>
                    {pendingOrdersCount} bon(s) d'achat nécessitent votre confirmation.
                  </Typography>
                </Box>
                <Button
                  variant="contained"
                ...
                <TableBody>
                  {filteredOrders.map((order) => (
                    <TableRow key={order.id}>
                      <TableCell sx={{ color: "#94a3b8" }}>{order.id}</TableCell>
                      <TableCell sx={{ color: "#94a3b8" }}>{order.supplier_name || "N/A"}</TableCell>
                      <TableCell sx={{ color: "#94a3b8" }}>{order.created_by_name || "N/A"}</TableCell>
                      <TableCell align="right" sx={{ color: "#10b981", fontWeight: 600 }}>
                        {parseFloat(order.total_amount || 0).toLocaleString("fr-FR", {
                          minimumFractionDigits: 2,
                        })} €
                      </TableCell>
                      <TableCell sx={{ color: "#94a3b8" }}>{order.expected_delivery_date || "N/A"}</TableCell>
                      ...
                      <TableCell align="center">
                        <Box sx={{ display: "flex", gap: 1, justifyContent: "center" }}>
                          <Tooltip title="Voir les détails">
                            <IconButton onClick={() => handleViewOrder(order)} size="small" sx={{ color: "#3b82f6" }}>
                              <VisibilityIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
                          {canApprove && normalizeStatus(order.status) === "pending" && (
                            <Button
                              variant="contained"
                              size="small"
      ...
      >
            ...
            <>
              <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Chip
                    label={statusConfig[normalizeStatus(selectedOrder.status)]?.label || selectedOrder.status}
                    sx={{
                      bgcolor: `${statusConfig[normalizeStatus(selectedOrder.status)]?.color || "#64748b"}20`,
                      color: statusConfig[normalizeStatus(selectedOrder.status)]?.color || "#64748b",
                      fontWeight: 600,
                    }}
                  />
                </Box>
        ...
        >
          <Box sx={{ display: "flex", gap: 1 }}>
            {canApprove && normalizeStatus(selectedOrder?.status) === "pending" && (
              <>
                <Button
...
export default BonAchatDemande;